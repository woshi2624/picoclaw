...
build_launcher_embedded() {
  local goos=$1 goarch=$2 agent_src=$3 out=$4
  shift 4

  local embed_dir="${WEB_BACKEND_DIR}/embedded"
  mkdir -p "$embed_dir"
  cp "${BUILD_DIR}/${agent_src}" "${embed_dir}/picoclaw.bin"

  info "Building ${out} with embedded agent (${goos}/${goarch})..."
  env GOOS="$goos" GOARCH="$goarch" "$@" \
    go build -tags "${GO_TAGS} embedded_binary" -ldflags "$LDFLAGS" \
    -o "${BUILD_DIR}/${out}" "./${WEB_BACKEND_DIR}"
...
# Normalize VERSION to "x.y.z.w" for go-winres (falls back to "0.0.0.0")
winres_version() {
  local v="${VERSION#v}"
  local parts
  parts="$(echo "$v" | grep -oE '^[0-9]+(\.[0-9]+){0,2}' || true)"
  [[ -z "$parts" ]] && { echo "0.0.0.0"; return; }
  local count
  count="$(echo "$parts" | awk -F. '{print NF}')"
  while [[ "$count" -lt 4 ]]; do parts="${parts}.0"; count=$((count + 1)); done
  echo "$parts"
}

cmd_winres() {
  local gowinres
  gowinres="$(go env GOPATH)/bin/go-winres"
  if ! command -v go-winres &>/dev/null && [[ ! -x "$gowinres" ]]; then
    info "Installing go-winres..."
    go install github.com/tc-hib/go-winres@latest
  fi
  # Prefer system PATH, fall back to GOPATH/bin
  if command -v go-winres &>/dev/null; then
    gowinres="go-winres"
  fi
  local ver
  ver="$(winres_version)"
  info "Generating Windows resources (version: ${ver})..."
  "$gowinres" make \
    --in "${CMD_DIR}/winres/winres.json" \
    --out "${CMD_DIR}/rsrc" \
    --product-version="${ver}" --file-version="${ver}"
  "$gowinres" make \
    --in "${WEB_BACKEND_DIR}/winres/winres.json" \
    --out "${WEB_BACKEND_DIR}/rsrc" \
    --product-version="${ver}" --file-version="${ver}"
}

cmd_winres_clean() {
  rm -f "${CMD_DIR}"/rsrc_windows_*.syso
  rm -f "${WEB_BACKEND_DIR}"/rsrc_windows_*.syso
}

cmd_package() {
  local goos=$1 goarch=$2 launcher_bin=$3
  local suffix=""
  [[ "$goos" == "windows" ]] && suffix=".exe"
  ...
  cp "${BUILD_DIR}/${launcher_bin}" "${stage_dir}/${pkg_name}/${PROJECT_NAME}${suffix}"
...
cmd_build() {
  ...
  for target in "${targets[@]}"; do
    ...
    # Embed icon and manifest into Windows executables
    ...
    # Build launcher with agent embedded (copies agent to web/backend/embedded/picoclaw.bin,
    # builds with embedded_binary tag, then removes embedded/ dir)
    local launcher_bin
    launcher_bin="$(binary_name "$goos" "$goarch" "${PROJECT_NAME}-launcher")"
    build_launcher_embedded "$goos" "$goarch" "$agent_bin" "$launcher_bin" \
      ${extra_env[@]+"${extra_env[@]}"}

    # Clean up .syso files (Windows only, generated by cmd_winres)
    if [[ "$goos" == "windows" ]]; then
      cmd_winres_clean
    fi

    # Package: zip contains only the launcher (renamed to picoclaw[.exe])
    cmd_package "$goos" "$goarch" "$launcher_bin"
  done
